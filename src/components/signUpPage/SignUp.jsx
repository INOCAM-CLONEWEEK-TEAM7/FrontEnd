...
function SignUp() {
  ...
  const [CheckUseTermBox, checkedUseTerm, setCheckedUseTerm] = useCheckBox('이용약관 동의 (필수)', 'https://newneek.notion.site/1e9ac1561fdb44109e2b154cf3b6a769');
  const [CheckPersonalBox, checkedPersonal, setCheckedPersonal] = useCheckBox('개인정보 수집/이용 동의 (필수)', 'https://newneek.notion.site/ec39ad284cfa4b7390fff1366236dc19');
  const [CheckMarketingBox, checkedMarketing, setCheckedMarketing] = useCheckBox('뉴스레터 및 마케팅 정보 수신 동의 (선택)', 'https://newneek.notion.site/97c0f3756cc54e3f9b201f8c3abd0dba');
  
  const [CheckAllBox, checkedAll, allMustsChecked] = useAllCheck([checkedUpper14,checkedUseTerm,checkedPersonal], [checkedMarketing], [setCheckedUpper14,setCheckedUseTerm,setCheckedPersonal], [setCheckedMarketing]);
  ...
  return (
    <form>
      ...
      <WithHelper msg='비밀번호가 달라요.' valid={checkPw===password} $margin='1rem 0'>
        <StyledInput type='password' placeholder='비밀번호 확인(8자 이상)' value={checkPw} onChange={handleCheckPwOnChange}/>
      </WithHelper>
